 ...
 */
@Component
public class SimpleNioServer implements NioServer{
	private static final Logger LOG = LoggerFactory.getLogger(SimpleNioServer.class);
	/**
	 * acceptor pool
	 */
	...
	public void start(final ServerConfig config) throws IOException {
		try {
			...
			.handler(config.getServerHandler() == null ? new LoggingHandler("nioserver", LogLevel.INFO) : config.getServerHandler())
